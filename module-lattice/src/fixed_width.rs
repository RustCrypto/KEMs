...
/// field and the Barrett-reduced [`core::ops::Mul`] on [`Elem`] would be wrong here.
...
pub struct FixedWidthPolynomial<F: Field, D: EncodingSize>(pub Array<FixedWidthInt<F, D>, U256>);
...
impl<F: Field, K: ArraySize, D: EncodingSize> From<Vector<F, K>> for FixedWidthVector<F, K, D> {
    fn from(vec: Vector<F, K>) -> Self {
        Self(vec.0.into_iter().map(FixedWidthPolynomial::from).collect())
    }
}

impl<F: Field, K: ArraySize, D: EncodingSize> From<FixedWidthVector<F, K, D>> for Vector<F, K> {
