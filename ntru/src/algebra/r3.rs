...
pub struct R3<Params: NtruCommon>(pub(super) Array<Small, Params::P>);
...
impl<Params: NtruCommon> R3<Params> {
    ...
    pub fn recip(&self) -> (Self, i32) {
        ...
        for i in 0..Params::P::USIZE {
            out[i] = Small::new_i8(sign * *v[Params::P::USIZE - 1 - i]);
        }
        (R3(out), i16_nonzero_mask(delta as i16))
    }
    ...
    pub fn short_from_list(data_in: &Array<u32, Params::P>) -> Self {
        ...
        let mut out = Array::<Small, Params::P>::default();
        for i in 0..Params::P::USIZE {
            out[i] = Small::new_i32(((l[i] & 3) - 1) as i32);
        }
        R3(out)
    }
}
